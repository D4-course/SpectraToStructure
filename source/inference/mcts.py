...
class Action:
    ...
    def add_child_state(self, s1, r, terminal, model, valuemodel):
        ...
        self.child_state = State(
            s1, r, terminal, self, self.parent_state.na, model, valuemodel
        )
...
class State:
    ...
    def __init__(self, mol_state, r, terminal, parent_action, na, model, valuemodel):
        ...
        self.mol_graph = (
            self.mol_state.molGraph
        )  # state (constructed molecule) + individual atoms
        ...
        self.action_mask = (
            self.mol_state.action_mask.copy()
        )  # better to have a standalone function also.
        ...
        self.model = model
        self.priors = model.predict_pi(
            self.mol_graph, self.action_mask, self.action_1_mask, self.index_mask
        )
        self.priors[(self.action_mask == 0).nonzero()] = -1e8
        self.priors = (
            torch.softmax(self.priors, dim=0) * torch.FloatTensor(self.action_mask)
        ).flatten()
        self.addPriorNoise()
    ...
    def select(self, c=0.01):
        ...
        random.seed()
        UCT = self.Q_value + (
            self.priors * c * np.sqrt(self.n + 1) / (self.N_value + 1)
        )
        # winner = argmax(UCT)
...
class MCTS:
            ...
            while not state.terminal:
                ...
                if hasattr(action, "child_state"):
                    ...
                    state = action.add_child_state(
                        s1, r, t, self.model, self.valuemodel
                    )  # expand
    ...
    def return_results(self, temp=1):
