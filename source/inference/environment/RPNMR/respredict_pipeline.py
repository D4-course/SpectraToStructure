...
def params():
    ...
    infile = td(
        "graph_conv_many_nuc_pipeline.data.{}.{}.{}.{:d}.{:d}.mol_dict.pickle".format(
            SPECT_SET, DATASET_NAME, MAT_PROPS, tgt_max_n, CV_I
        )
    )
...
def train(infile, outfile, seed):
    ...
    dl_test = torch.utils.data.DataLoader(
        ds_test, batch_size=BATCH_SIZE, shuffle=True, pin_memory=True
    )
    ...
    pickle.dump(
        metadata, open(os.path.join(CHECKPOINT_DIR, MODEL_NAME + ".meta"), "wb")
    )

    graph_conv_many_nuc_util.generic_runner(
        net,
        optimizer,
        scheduler,
        criterion,
