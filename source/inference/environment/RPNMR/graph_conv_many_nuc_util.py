...
class ModelTrial:
    def __init__(self, training_loader, validation_loader, hparams):
    ...
    def build_model(self, hparams):
        ...
        Return the model
        """
        net = nets.GraphVertExtraLinModel(
            g_feature_n,
            [INT_D] * LAYER_N,
            resnet=USE_RESNET,
            noise=INIT_NOISE,
...
default_feat_mat_args = dict(feat_distances=False, feat_r_pow=None)
...
def run_epoch(
    ...
    for i_batch, (adj, vect_feat, mat_feat, vals, mask) in iterator:
        t1 = time.time()
        if print_shapes:
            print(
                "adj.shape=",
...
def generic_runner(
    ...
    for epoch_i in tqdm(range(MAX_EPOCHS)):
        ...
        if writer is not None:
            writer.add_scalar(
                "train_loss",
                train_res["running_loss"] / train_res["total_points"],
                epoch_i,
            )
...
class PredModel(object):
    ...
    def pred(self, rdmols, values, BATCH_SIZE=32, debug=False, prog_bar=False):
