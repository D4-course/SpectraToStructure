...
class Model(object):
    def __init__(self, meta_filename, checkpoint_filename, USE_CUDA=False):
        ...
        if self.USE_CUDA:
            ...
            self.model.load_state_dict(
                torch.load(modelState, map_location=torch.device("cpu")), strict=False
            )
        self.net = self.model
        self.net.eval()
    ...
    def pred(self, rdmols, values, BATCH_SIZE=32, debug=False):
        ...
        m_pos = 0
        # removing tqdm :
        for i_batch, (adj, vect_feat, mat_feat, vals, mask) in enumerate(
            dl
        ):  # enumerate(tqdm(dl))

            if debug:
                print(
                    adj.shape, vect_feat.shape, mat_feat.shape, vals.shape, mask.shape
                )
