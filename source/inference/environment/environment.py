...
class Env:
    ...
    def __str__(self):
        return (
            "Current Env State : " + str(self.state) + " MolForm : " + str(self.molForm)
        )
    ...
    def reward(self, state=None, target=None):
        ...
        if splits.count("Q") > Q:
            ...
            splits.count("Q") == Q and splits.count("T") == T and splits.count("D") > D
        ):
            return 0

        # predicted_nmr = ray.get(self.episode_actor.predict.remote(Chem.MolToSmiles(mol)))
        predicted_nmr = self.episode_actor.predict(Chem.MolToSmiles(mol))
        if predicted_nmr == -1:
            return 0
    ...
    def isTerminal(self, state: MolState = None):
        try:
            ...
            if splits.count("Q") > Q:
                ...
                splits.count("Q") == Q
                and splits.count("T") == T
                and splits.count("D") > D
            ):
                return True
            elif (
                splits.count("Q") == Q
            ...
            if S > splits.count("S"):
                ...
                S == splits.count("S")
                and D == splits.count("D")
                and T > splits.count("T")
            ):
                return True
